...
import { getBlogSlugs, getBlogPost, getBlogPosts } from "@/lib/content";
...
export async function generateMetadata({
  ...
  if (!post) return {};
  return {
    title: `${post.title} | AdviserGPT Blog`,
...
export default async function BlogPostPage({
  ...
  const otherBlogs = getBlogPosts()
    .filter((p) => p.slug !== slug)
    .slice(0, 2);

  return (
    <>
      <main style={{ paddingTop: 88, background: "#fafaf9", minHeight: "100vh", overflow: "visible" }}>
        <Navbar />

        {/* Light header — back link, date/category, title, author */}
        <header
          style={{
            background: "#fafaf9",
        ...
        >
          <div style={{ maxWidth: 1100, margin: "0 auto" }}>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.5, color: "#555", marginBottom: 0 }}>
              {post.author}
            </p>

            {/* Hero: full width of content+TOC area, 16:9 card */}
                ...
                {post.heroImage ? (
                  ...
                  >
                    <Image
                      ...
                      sizes="(max-width: 1148px) 100vw, 1052px"
                    />
                  </div>
                ) : (
                  <div
