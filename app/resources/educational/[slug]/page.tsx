...
export function generateStaticParams() {
  return getEducationalSlugs().map((slug) => ({ slug }));
}
...
export async function generateMetadata({
  ...
  const post = getEducationalPost(slug);
  if (!post) return {};
  return {
...
export default async function EducationalPostPage({
  params,
}: {
  params: Promise<{ slug: string }>;
}) {
  ...
  const otherPosts = getEducationalPosts()
            ...
            >
              {otherPosts.map((p) => (
                <ResourceCardSmall
                  ...
                  href={`/resources/educational/${p.slug}`}
                />
              ))}
            </div>
          </section>
